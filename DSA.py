...
PROMPT = (
    "You are an expert coding and MCQ assistant. The screenshots contain either a coding problem or a multiple-choice question.\n"
    "Analyze all screenshots together as one combined context.\n\n"
    "DIRECTIONS:\n"
    "1. If it is an MCQ: Output ONLY the correct option letter (e.g., 'a', 'b', 'c', or 'd') in lowercase. NO punctuation, NO labels, just the letter.\n"
    "2. If it is a coding question: Output ONLY the raw executable code.\n\n"
    "ABSOLUTE RULES for Coding Questions:\n"
    "- No comments, no markdown, no explanations, no preamble.\n"
...
def deliver_answer(answer: str):
    if AUTO_TYPE:
        # We start in a paused state and wait for the user to press a+s or Esc
        global is_paused
        is_paused = True
        pause_event.clear()

        print(f"⌨️   AI is ready! Click into the answer field and press 'a+s' or 'Esc' to start typing.", flush=True)
        print(f"    a+s or Esc → START/Pause/Resume  |  k+x → stop completely", flush=True)
