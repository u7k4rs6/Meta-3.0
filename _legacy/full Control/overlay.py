...
class OverlayWindow:
    def __init__(self):
        ...
        self.sysaudio_active = False

        # Callbacks set by main.py
        self.on_send          = None   # fn(text: str)
        self.on_clear         = None   # fn()
        self.on_mic_start     = None   # fn()
        self.on_mic_stop      = None   # fn() → triggers transcription → send
        self.on_sysaudio_toggle = None # fn(active: bool)
    ...
    def _build_window(self):
        ...
        # System audio toggle
        self.sysaudio_btn = tk.Label(header, text=" 🔊 ", bg='#0e0e1a', fg='#333360',
                                     font=('Segoe UI', 10), cursor='hand2')
        self.sysaudio_btn.pack(side='right')
        self.sysaudio_btn.bind('<Button-1>', lambda e: self._toggle_sysaudio())
        self.sysaudio_btn.bind('<Enter>',    lambda e: self.sysaudio_btn.config(fg='#e0a050') if not self.sysaudio_active else None)
        self.sysaudio_btn.bind('<Leave>',    lambda e: self.sysaudio_btn.config(fg='#e0a050') if self.sysaudio_active else self.sysaudio_btn.config(fg='#333360'))
        ...
        # Mic button
        self.mic_btn = tk.Label(input_row, text=' 🎤 ', bg='#0e0e1a', fg='#333360',
        ...
        # Hint
        tk.Label(inner, text="Hold 🎤 to record mic  •  🔊 to listen for interviewer  •  m+n hide  •  k+c clear",
                 bg='#0e0e1a', fg='#1e1e40', font=('Segoe UI', 8)
                 ).pack(pady=(6, 0))

        # Position & drag
        sw = self.root.winfo_screenwidth()
        self.root.geometry(f"580x560+{sw - 610}+24")
    ...
    # ── Mic press/release (hold to record) ───────────────────────────────────
    def _mic_press(self, event):
        self.mic_active = True
        self.mic_btn.config(fg='#ff5555')
    ...
    def _mic_release(self, event):
        if not self.mic_active:
            return
        self.mic_active = False
        self.mic_btn.config(fg='#333360')
        if self.on_mic_stop:
            threading.Thread(target=self.on_mic_stop, daemon=True).start()

    # ── System audio toggle ───────────────────────────────────────────────────
    def _toggle_sysaudio(self):
        self.sysaudio_active = not self.sysaudio_active
        if self.sysaudio_active:
            self.sysaudio_btn.config(fg='#e0a050')
        else:
            self.sysaudio_btn.config(fg='#333360')
        if self.on_sysaudio_toggle:
            threading.Thread(
                target=self.on_sysaudio_toggle,
                args=(self.sysaudio_active,),
                daemon=True
            ).start()
    ...
    def set_mic_transcribing(self, state: bool):
        if not self.root:
            return
        def _update():
            if state:
                self.mic_btn.config(fg='#f0a050')
    ...
    def set_input_text(self, text: str):
        """Set the input field text (used after mic transcription)."""
        if not self.root:
            return
        def _update():
